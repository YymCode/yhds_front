<template>
    <div class="full-height">
      <Breadcrumb separator=">">
        ...
        <Breadcrumb-item href="/keywords/perr">同行关键词</Breadcrumb-item>
  ...
  export default {
    ...
    methods:{
      ...
      filterWordList() {
        ...
        this.api.filterIndustryKeywords.filter({
          reqData:{
            "limit": this.limit,
            "offset": this.offset,
            "sort": this.sortType,
            "order": this.order,
            "kwqId":this.$route.params.kwqId,
            "ikwKeywords":this.formFilter.keyValue,
      ...
      exportData() {
        if(this.tableData.length){
          window.location.href=process.env.API_ROOT+'/alibaba/industryKeywords/exportIndustryKeywords?kwqId='+this.$route.params.kwqId
        }else{
          this.$Notice.warning({
            title: '提示',
            desc: '暂无可导出数据'
      ...
      chooseRow(selection) {
        ...
        for(var i=0;i<selection.length;i++){
          arr.push(selection[i].ikwId)
          list.push(selection[i].ikwKeywords)
        }
...
//    },
      ...
      addRowClassName (row, index) {
        if(this.thesaurusString.indexOf(row.ikwId)!==-1){
          return 'row-blue';
        }
        return ''
      },
      ...
      loadWordList() {
        ...
        this.api.findByQueue.list({
          ...
          for(var i=0;i<res.data.rows.length;i++){
          	...
          	if (res.data.rows[i].inSelf == 1) { 
				res.data.rows[i]._disabled = true; 
			} 
          }
