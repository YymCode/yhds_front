<template>
  <div class="full-height">
    ...
    <div class="main mainbody">
        <!--搜索-->
       <div>
         <Select v-model="word_modelsta" style="width:140px;margin-right: 15px;" @on-change="word_allstate()">
           <Option v-for="item in word_cityList" :value="item.value" :key="item.id" style="z-index: 10000">{{ item.label }}</Option>
         </Select>
         <Select v-model="word_modelall" style="width:140px;margin-right: 15px;" @on-change="word_allshow()">
           <Option v-for="item in word_cityshow" :value="item.value" :key="item.value">{{ item.label }}</Option>
         </Select>

         <Input v-model="word_modelval" placeholder="输入关键词" style="width:230px; margin-right:30px;"></Input>
         ...
         <!--<div class="xuan">-->
           <!--<Select v-model="word_starlevel" style="width:140px;margin-right:15px;" @on-change="seleatrr" >-->
             <!--<Option v-for="item in word_starlist" :value="item.value" :key="item.id">{{ item.label }}</Option>-->
           <!--</Select>-->

           <!--<Select v-model="word_nextlevel" style="width:140px;" @on-change="selechange">-->
             <!--<Option v-for="item in word_nextlist" :value="item.value" :key="item.id">{{ item.label }}</Option>-->
           <!--</Select>-->
         <!--</div>-->
       </div>
       <div class="word_btnstyle">
        <Button type="primary" shape="circle" @click="word_goadd">添加关键词</Button>
        <Button type="primary" shape="circle" @click="word_addgroup">新建分组</Button>
        <div>
          <span class="word_btnspan">
            <Button type="text"  id="word_btn1">
            操作
            <Icon type="arrow-down-b"></Icon>
           </Button>
           <div class="word_btnhide">
            <ul>
              <li @click="word_deletabel()">删除</li>
              <li @click="word_suspended">暂停非策略词</li>
              <li @click="word_pulltabel()">修改所属组</li>
              <li @click="word_chooci">加入到已有策略</li>
              <li @click="word_removekeys">从策略移出</li>
          ...
          <!--<Button type="primary" shape="circle" @click="word_exportData">导出关键词</Button>-->
          <Button type="primary" shape="circle" @click="addmodalfun()" >添加竞价策略</Button>
          <Button type="primary" shape="circle" @click="word_synchronous">同步关键词</Button>
      ...
      <div class="word_bodyconten">
        <div>
          <!--<div class="word_haed">分组名称</div>-->
            ...
            <!--</ul>-->
          <Menu :theme="word_theme2"  style="width: 100%" :active-name="1" ref="menu" :open-names="$route.query.type==1||$route.query.type==4?['3']:['1']">
            <MenuItem :name="$route.query.type?999:1" @click.native="word_allbar">
            ...
            <Submenu name="3">
              ...
              <MenuItem :name="$route.query.type==1?1:3-3" @click.native="listType='高曝低点'">高曝光低点击</MenuItem>
              <MenuItem :name="$route.query.type==4?1:3-4" @click.native="listType='高花费'">高花费</MenuItem>
            </Submenu>
            <Submenu name="4">
              <template slot="title">
            ...
            <MenuItem :name="$route.query.type==2?1:5" @click.native="listType='竞价词'">
            	<i class="ivu-icon ivu-icon-plus-round"></i>
              建议竞价词
            </MenuItem>
            <MenuItem name="$route.query.type==3?1:6" @click.native="listType='策略词'">
            	<i class="ivu-icon ivu-icon-plus-round"></i>
              策略词
            </MenuItem>
          </Menu>
        ...
        <div>
          <div style="height:calc(100% - 40px)" ref="mainHeight">
            <Table  :loading="word_loading" :height="tableHeight" border stripe ref="selection" :columns="word_columns" :data="word_tabeldata"  @on-selection-change="word_tabelchange" @on-sort-change="word_sortChange" style="height:100%"></Table>
          </div>
        ...
        <ul>
          ...
          <li v-for="item in word_promall">
            <Row type="flex" justify="center" align="middle" class="word_code-row-bg">
              ...
              <Col span="4" style="cursor: pointer; color: #00a1ff;text-align: right" @click.native="word_editorurl(item.proId)">编辑产品</Col>
            </Row>
          </li>
        </ul>
    ...
    <Modal v-model="word_removedata" width="260" :styles="{top: '60px'}">
      <p slot="header" style="color:#f60;text-align:center">
        <Icon type="information-circled"></Icon>
        <span>是否确认删除</span>
      </p>
      <p>
        确认删除关键词组 {{word_removedataName}}
      </p>

      <div slot="footer">
        <Button type="error" size="large" long @click="word_confirmdele">删除</Button>
      </div>
    </Modal>
    <!--新建策略模态框-->
    <Modal ref="addmodal" :title="modetext" v-model="addmodal" width="600" :styles="{top: '60px'}" @on-ok="addnew">
    	<div class="modeconten">
    		<p>
    
    			<span class="nameofthe">策略名称：</span>
    			<Input v-model="strategyname" placeholder="请输入策略名称" style="width: 400px;height: 35px;"></Input>
    		</p>
    		<p>
    ...
    <!--投放时间模态框-->
    <Modal title="选择投放时间(北京时间)" v-model="puttime" width="700" :styles="{top: '60px'}" @on-ok="castok">
    ...
    <!--修改时间弹框 -->
    <Modal v-model="timemodal" title="修改时间" width="400" @on-ok="choopullok">
    ...
    <!--修改分段时间弹框 -->
    <Modal v-model="period" title="设置分段出价(北京时间)" width="770" @on-ok="periodok" :styles="{top: '100px'}">
    ...
    <!--修改分段值弹框 -->
    <Modal v-model="periodxxx" title="设置当前分段出价(北京时间)" width="440" @on-ok="perinewadd" :styles="{top: '100px'}">
    ...
    <!--添加关键词弹框-->
    <Modal v-model="keywordsdata" title="添加关键词" @on-ok="keywordsok" width="820" :styles="{top: '100px'}">
...
//              选择策略
      ...
      mounted(){
        ...
        this.word_allgroup();
        this.tableHeight =this.$refs.mainHeight.offsetHeight -40;
        this.$nextTick(() => {
...
//                if(this.seledata2=="差价"){pri=this.difference;pri2="";}else{pri="";pri2=this.difference2;};
      ...
      addmodalfun(){
        this.keywrodnum=this.data4.length;
        this.addpullsta="0";
...
//          删除关键词所属策略
          ...
          pause(){
              ...
              var list=this.word_removeId.join(",");
              this.api.stopKeywords.data({reqData:{pfpPkwIds:list}}).then((res) => {
                if(res.data.status==200){
                  this.$Notice.success({
                    title: '暂停成功',
                  });
...
//          表格选中
          word_tabelchange(selection){
              ...
              this.word_deleId=[];
							...
							this.mobiledata=[];
              this.word_removeId=[];
              //this.mobilekeys(selection[selection.length-1].pfpPkwKeywords,selection[selection.length-1].pfpPkwId)
              for(var i=0;i<selection.length;i++){
                ...
                this.mobilekeys(selection[i].pfpPkwKeywords,selection[i].pfpPkwId)
              }
...
//          表格排序
          ...
          word_init(){
            ...
            if(this.listType=='策略词'){
            	this.api.findPkwStrategy.find({reqData:{
	                   ...
	                   obj= {
	                     ...
	                     render: (h, params) => {
	                       return h('div', {
	                         ...
	                         on:{
	                           click: () => {
	                               this.word_promoteshow(params.row.pfpPkwKeywords,params.row.pfpPkwId,params.row.pfpAlibabaKeyId);
	...
	//                   预估排名
            ...
            }else if(this.listType=='高曝低点'){
            	this.api.findUpWords.find({reqData:{
	              pfpPkwKeywords:this.word_modelval,
	//              pfpPkwStrategy:this.word_modelall,
	              pfpPkwStrategy:this.word_modelall,
	              iaAccurate:sat,
	...
	//                   预估排名
            ...
            }else if(this.listType=='高花费'){
            	this.api.findUpCost.find({reqData:{
	              pfpPkwKeywords:this.word_modelval,
	//              pfpPkwStrategy:this.word_modelall,
	              pfpPkwStrategy:this.word_modelall,
	              iaAccurate:sat,
	...
	//                   预估排名
            ...
            }else if(this.listType=='竞价词'){
            	this.api.findMonthWork.find({reqData:{
	              pfpPkwKeywords:this.word_modelval,
	//              pfpPkwStrategy:this.word_modelall,
	              pfpPkwStrategy:this.word_modelall,
	              iaAccurate:sat,
...
//          查看分组
          ...
          word_Modifyok(){
            if(this.word_nwenamevalue){
              ...
              this.$refs.word_Modify.visible = true;
              this.word_Modify=true
              this.$Notice.info({
                title: '请输入分组名',
              });
          ...
          word_removlist(val){
            this.word_removedata=true;
            this.word_removedataName=val;
          },

          word_confirmdele(){
            this.api.p4pdele.find({reqData:{tagName:this.word_removedataName}}).then((res) => {
              if (res.data.status == 200) {
                this.$Notice.success({
                  title: '删除成功',
                });
                this.word_removedata=false;
                this.word_viewgrouping();
              }
            })
          },
...
//          关键词产品推广
          ...
          word_chooseok(){
              if(this.word_animal){
                var list=this.word_removeId.join(",");
                this.api.hotaddKeywordsToStrat.data({reqData:{keywordIds:list,stId:this.word_animal,strategyWordsSource:"thesaurus"}}).then((res) => {
                  if (res.data.status == 200) {
                    this.$Notice.success({
                      title: '添加成功',
                    });
